...
export default async function HealthOverviewPage() {
  ...
  const muscleStates = MUSCLE_GROUPS.reduce(
    (acc, g) => {
      ...
      acc[g] = found ?? { group: g, daysSince: null, hoursSince: null, effectiveHoursSince: null, lastTrainedISO: null, lastSets: [] };
      return acc;
    },
    ...
    <Stagger className="space-y-8">
      <section className="space-y-3">
        <div className="flex items-baseline justify-between">
          ...
          <GarminSyncedChip syncedAt={wellnessToday?.syncedAt ?? null} />
        </div>
        <WellnessTodayCards
          hr={{
